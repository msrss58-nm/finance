...
- **עדכון 03/08/2026 — Version 1.2 (UI/UX Refinement, טרם ב-commit)**: טאב "תנועות" הוסר מהניווט התחתון — **4 טאבים כעת** (בית/תובנות/קטגוריות/הגדרות); צפייה בכל התנועות היא שורת מערכת "📋 כל התנועות" בראש מסך קטגוריות. מסך "הגדרות" פוצל מרשימה שטוחה לתפריט 7 נושאים (עם תיאור קצר לכל אחד) + מסכי-משנה ייעודיים עם כפתור "← חזרה". סה"כ **6 מסכים כעת** (5 טאבים לוגיים + מסך-משנה אחד ללא טאב עצמאי — ראו "המסכים והמודולים" למטה, מעודכן). `APP_VERSION` בקוד: `'1.2.0 Preview'`. ראו CURRENT_STATUS.md/CHANGELOG.md, סעיף "Version 1.2".
