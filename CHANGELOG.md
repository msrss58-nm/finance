...
עודכנו CURRENT_STATUS.md/CHANGELOG.md (רשומה זו)/PROJECT_CONTEXT.md/TODO.md. `APP_VERSION`: `'1.2.0 Preview'` → `'1.3.0 Preview'`. `getForecastWarning`/`getBiggestUpcomingCharge`/`getProjectedBalanceAfterUpcoming` מוגדרות-אך-לא-בשימוש-חי (deferred, לא נמחקו). **Version 1.3 ממתינה לאישור Commit מפורש.**
